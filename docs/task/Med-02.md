...
Kryteria akceptacji:
- [x] klasa `Patient` leży w pakiecie `model` pod pakietem klasy głównej
- [x] `birthday` jest typu `LocalDate`, a nie `String`
- [x] klasa nie ma nad sobą żadnej adnotacji
- [x] `mvn clean package` kończy się na `BUILD SUCCESS`
...
Kryteria akceptacji:
- [x] nad klasą stoi `@Repository`
- [x] pola listy i licznika są `private final`
- [x] `findById` i `findByEmail` zwracają `Optional`, nie `null`
- [x] `findAll` zwraca kopię listy, a nie listę trzymaną w repozytorium
- [x] projekt się buduje
...
Kryteria akceptacji:
- [x] cztery pliki w pakiecie `notification`, projekt się kompiluje
- [x] uruchomienie kończy się na `APPLICATION FAILED TO START`
- [x] w notatce jest pełny blok `Description:` z komunikatu
- [x] w notatce są dwa zdania wyjaśnienia, nie sam wklejony tekst
...
Kryteria akceptacji:
- [x] aplikacja wstaje, w logu jest `Started MedicalClinicApplication`
- [x] na konsoli pojawia się wiersz zaczynający się od `e-mail do`
- [x] w notatce jest jedno zdanie o tym, co robi `@Primary`
...
Kryteria akceptacji:
- [x] w notatce jest zdanie o tym, że wersja z adnotacją nad polem wstaje i wysyła e-mail,
  czyli kanał domyślny, a nie ten, o który prosiłeś, i nie zgłasza przy tym błędu
- [x] `VisitReminderService` ma konstruktor napisany ręcznie, bez `@RequiredArgsConstructor`
- [x] `@Qualifier("smsReminderSender")` stoi przy parametrze konstruktora
- [x] po uruchomieniu na konsoli jest wiersz zaczynający się od `SMS do`
- [x] `EmailReminderSender` nadal ma `@Primary`
